...
fun EnergyScreen(
    ...
    // Pre-fill with today's carried-forward intake (the last value entered, still in effect for
    // today's diet XP) so a new day starts from yesterday's number — edit it unless it's right.
    // -1 = nothing entered anywhere yet → blank. An explicit 0 is a fasting day and shows as "0".
    ...
    val est = Calories.estimate(
        liveProfile, state.today.toDayData(state.customSpecs).copy(weightKg = 0.0),
